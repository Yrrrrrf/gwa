...
Deno.test("🗄️ DB Computed Stats (Events)", async (t) => {
  await withSurrealEnv("Computed Stats Validation", async ({ surreal, cleanup }) => {
    ...
    await t.step("C1: Comment updates item rating and count", async () => {
      ...
      const res0 = await surreal.sql(`CREATE ${itemId} SET title='Stat Test', status='active', tags=[];`);
      ...
      // 2. Initial check
      let res = await surreal.sql(`SELECT rating, comment_count FROM ${itemId};`);
      ...
      await surreal.sql(`RELATE ${userId}->comment->${itemId} SET rating=5, body='Great!';`);
      ...
      const actualResUpdate = res.find((r: any) => !(r.result?.database && r.result?.namespace));
      ...
      await surreal.sql(`RELATE user:bob->comment->${itemId} SET rating=1, body='Bad!';`);
      
      await new Promise(r => setTimeout(r, 100));

      res = await surreal.sql(`SELECT rating, comment_count FROM ${itemId};`);
      const actualResUpdate2 = res.find((r: any) => !(r.result?.database && r.result?.namespace));
